...
import com.github.tranchis.xsd2thrift.marshal.Protobuf2Marshaller;
import com.github.tranchis.xsd2thrift.marshal.Protobuf3Marshaller;
...
public class Main {
    ...
    private static String usage = ""
            + "Usage: java -jar xsd2thrift.jar [--thrift] [--protobuf] [--filename=FILENAME]\n"
            ...
            + "  --directory=DIRNAME         : store the result in DIRNAME\n"
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args.length == 0 || args[args.length - 1].startsWith("--")) {
            ...
            while (correct && i < args.length - 1) {
                if (args[i].equals("--thrift")) {
                ...
                } else if (args[i].equals("--protobuf") || args[i].equals("--proto2")) {
                    if (im == null) {
                        im = new Protobuf2Marshaller();
                        xp.addMarshaller(im);
                        writer.setMarshaller(im);
                ...
                } else if (args[i].equals("--proto3")) {
                    if (im == null) {
                        im = new Protobuf3Marshaller();
                        xp.addMarshaller(im);
                        writer.setMarshaller(im);
                        writer.setDefaultExtension("proto");
                    } else {
                    ...
                    param = args[i].split("=")[1];
                    boolean nestEnums = Boolean.parseBoolean(param);
