...
public class OutputWriter {
    ...
    private OutputStream getNamespaceSpecificStream(String cleanedNamespace) throws IOException {
        if (!streams.containsKey(cleanedNamespace)) {
            File file = new File(directory() + cleanedNamespace.replace(".", "_") + "." + defaultExtension);
            OutputStream os = new FileOutputStream(file);
            streams.put(cleanedNamespace, os);
            os.write(marshaller.writeHeader(cleanedNamespace, file.getName()).getBytes());
        }
    ...
    private void initializeOutputStream() throws IOException {
        if (splitBySchema) {
            ...
            if (filename == null) {
                os = System.out;
                os.write(marshaller.writeHeader(defaultNamespace, null).getBytes());
            } else {
                os = new FileOutputStream(directory() + filename);
                os.write(marshaller.writeHeader(defaultNamespace, filename).getBytes());
            }
        }
    }
